<?php
class ControllerPrilCommutation extends Controller {
	public function index() {
		...
		if ($usersideComm['Result'] == 'OK' && !empty($usersideComm['data'])) {
				...
				switch ($usersideComm['data']['finish']['object_type']) {
					case "switch":
						$switchData = array(
							...
							'action'      => 'get_data',
							'object_type' => 'switch',
							'object_id'   => $usersideComm['data']['finish']['object_id'],
						);
						$usersideswitch = object_to_array(json_decode(create_ticket($switchData, USERSIDE)));
