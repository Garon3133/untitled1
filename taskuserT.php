<?php
class ControllerPrilTaskusert extends Controller {
	public function index() {
		
		require_once 'function.php';
		
		$user_id = '70';
		$task_date = '2020-09-14';
		$key_pril = '100-200';
		...
		if ($userside_task['Result'] == 'OK') {
				if ($userside_task['count'] != "0") {
					...
					foreach ($task_list_arr as $task_list) {
						...
						if ($taskres['Result'] == 'OK' && $taskres['Data']['type']['id'] != '291') {
					...
					$checkTasks = $userside_task['count'] - $delTaskCount;
					if($checkTasks == '0'){
						print_r("Заданий нет.");
					}
				} else {
